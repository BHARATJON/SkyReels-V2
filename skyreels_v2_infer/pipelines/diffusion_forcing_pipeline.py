...
class DiffusionForcingPipeline:
    ...
    def __init__(
        ...
        # Optionally wrap transformer with DataParallel when multiple GPUs are available
        self._use_dataparallel = bool(use_dataparallel) and torch.cuda.is_available() and torch.cuda.device_count() > 1 and (not use_usp)
    ...
    @property
    def transformer_module(self):
        """Return the underlying transformer module (unwrap DataParallel if used)."""
        return getattr(self.transformer, "module", self.transformer)

    @property
    def transformer_dtype(self):
        return getattr(self.transformer_module, "dtype", torch.bfloat16)
    ...
    def encode_image(
        ...
        causal_block_size = self.transformer_module.num_frame_per_block
    ...
    def extend_video(
        ...
        prompt_embeds = self.text_encoder.encode(prompt).to(self.transformer_dtype)
        if self.do_classifier_free_guidance:
            negative_prompt_embeds = self.text_encoder.encode(negative_prompt).to(self.transformer_dtype)
        if self.offload:
        ...
        for i in range(n_iter):
            ...
            self.transformer_module.to(self.device)
